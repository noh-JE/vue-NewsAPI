    ...
    <ul>
      <li v-for="item in listItems" v-bind:key="item.id">
        ...
        <div class="info">
          ...
          <div v-if="item.user">
            <span>작성자 : </span>
            <router-link class="user" v-bind:to="`/user/${item.user}`">{{ item.user }}</router-link> <span>(points : {{item.points}})</span>
          </div>
          <div v-else>
            <a class="user" v-bind:href="`${item.url}`" target="_blank">{{ item.domain }}</a>
          </div>
        </div>
      </li>
    </ul>
